...
# A winget table separator: a line made only of dashes/spaces, either a single
# continuous run (French locale observed) or dash runs aligned under each
# header column (English default) — so allow interior spaces.
DASHES = re.compile(r"^[ -]+$")
...
def _run_winget(args: list[str]) -> subprocess.CompletedProcess[str]:
    ...
    ``subprocess.run`` raises ``FileNotFoundError`` (an :class:`OSError`) if
    winget is not installed or its App Execution Alias is broken; convert that
    into a ``WingetError`` too so callers get one failure type.
    """
    try:
        proc = subprocess.run(
            [_winget_path(), *args],
        ...
        raise WingetError(f"could not run winget: {exc}") from exc
