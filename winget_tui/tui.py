...
class UpdateTable(DataTable):
    ...
    BINDINGS = [
        Binding("enter", "select_cursor", "Update"),
    ]
...
class WingetTuiApp(App):
    ...
    # No theme switching, command palette, or dark-mode toggles: the app is
    # always rendered in one fixed scheme, so the palette (and its `^p` footer
    # entry) is disabled — smaller footer, no way to change themes.
    ENABLE_COMMAND_PALETTE = False

    # Ordered most→least important left-to-right in the footer. The focused
    # DataTable's `enter` (Update) binding renders first, so this list governs
    # everything after it. Labels stay short so the footer fits narrow widths.
    BINDINGS = [
        Binding("space", "toggle_selection", "Toggle"),
        Binding("a", "select_all", "All"),
        Binding("n", "clear_selection", "None"),
        Binding("b", "toggle_blacklist", "Block"),
        Binding("h", "toggle_blacklist_view", "Hidden"),
    ...
    def _load_packages(self) -> None:
        ...
        except Exception as exc:  # WingetError, OSError, unexpected parser issues
